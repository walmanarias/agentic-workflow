...
# Usage: bash scripts/install.sh /path/to/target/repo [--force]
set -euo pipefail
...
if [ -z "$TARGET" ]; then
  echo "Usage: bash scripts/install.sh /path/to/target/repo [--force]" >&2
  exit 1
fi
if [ ! -d "$TARGET" ]; then
...
Workflow: /plan -> /spec -> /tdd -> /implement -> /e2e -> /review -> /ship.
MD
