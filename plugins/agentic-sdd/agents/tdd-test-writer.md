...
2. Detect the test runner and conventions (Jest is the default; also support Vitest if the repo uses it). Match existing file placement (`*.test.ts`, `__tests__/`, etc.). For framework-specific test idioms, load the matching stack skill (`react-expert`, `remix-expert`, `angular-expert`, `nestjs-expert`, `dotnet-expert`, …) or `stack-testing-recipes`; meta-framework skills (`nextjs-expert`, `remix-expert`) pair with `react-expert` for component tests.
3. Write tests that:
