...
It loads the relevant stack-expert skill (react / react-native / node-backend / nestjs / nextjs / django / fastapi / flask / dotnet / avalonia / maui / database) plus `tdd-workflow` and `clean-code`. Write the minimum code to pass the failing tests, then refactor with tests staying green. Never edit a test to pass; if a test seems wrong, stop and flag it. Run full tests + lint + type-check before declaring done. Show the diff and passing output, then hand off to `/review`.
